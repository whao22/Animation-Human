...
def transaction(args):
    ...
    while True:
        for bone_matrix,location in zip(p,t):
            bone_euler=rotation_matrix_to_euler_angle(bone_matrix)#/np.pi*180
...
def deformation(rotations):
    ...
    bone_rest_matrix[0]=pelvis_rest=np.array([[1,0,0],[0, 0.9939, 0.1104],[0,-0.1104,0.9939]])
    bone_rest_matrix[1]=chest_rest=np.array([[1,0,0],[0,1,0],[0,0,1]])
    bone_rest_matrix[2]=chest2_rest=np.array([[1,0,0],[0,1,0],[0,0,1]])
    bone_rest_matrix[3]=chest3_rest=np.array([[1,0,0],[0,1,0],[0,0,1]])
    bone_rest_matrix[4]=chest4_rest=np.array([[1,0,0],[0,1,0],[0,0,1]])
    bone_rest_matrix[5]=neck_rest=np.array([[1,0,0],[0,1,0],[0,0,1]])
    bone_rest_matrix[6]=head_rest=np.array([[1,0,0],[0,1,0],[0,0,1]])

    bone_rest_matrix[7]=rcollar_rest=np.array([[0,-1,0],[1,0,0],[0,0,1]])
    bone_rest_matrix[8]=rshoulder_rest=np.array([[0,-1,0],[1,0,0],[0,0,1]])
    bone_rest_matrix[9]=relbow_rest=np.array([[0,-1,0],[1,0,0],[0,0,1]])
    bone_rest_matrix[10]=rwrist_rest=np.array([[0,-1,0],[1,0,0],[0,0,1]])

    bone_rest_matrix[11]=lcollar_rest=np.array([[0,1,0],[-1,0,0],[0,0,1]])
    bone_rest_matrix[12]=lshouder_rest=np.array([[0,1,0],[-1,0,0],[0,0,1]])
    bone_rest_matrix[13]=lelbow_rest=np.array([[0,1,0],[-1,0,0],[0,0,1]])
    bone_rest_matrix[14]=lwrist_rest=np.array([[0,1,0],[-1,0,0],[0,0,1]])

    bone_rest_matrix[15]=rhip_rest=np.array([[-1,0,0],[0,-1,0],[0,0,1]])
    bone_rest_matrix[16]=rknee_rest=np.array([[-1,0,0],[0,-1,0],[0,0,1]])
    bone_rest_matrix[17]=rankle_rest=np.array([[-1,0,0],[0,-1,0],[0,0,1]])
    bone_rest_matrix[18]=rtoe_rest=np.array([[-1,0,0],[0,-1,0],[0,0,1]])
    bone_rest_matrix[19]=lhip_rest=np.array([[-1,0,0],[0,-1,0],[0,0,1]])
    bone_rest_matrix[20]=lknee_rest=np.array([[-1,0,0],[0,-1,0],[0,0,1]])
    bone_rest_matrix[21]=lankle_rest=np.array([[-1,0,0],[0,-1,0],[0,0,1]])
    bone_rest_matrix[22]=ltoe_rest=np.array([[-1,0,0],[0,-1,0],[0,0,1]])
    

    # poses matrix
    for j,rotation in enumerate(rotations):
        for i in range(len(rotation)):
            bone_matrix=Rotation.from_euler(seq="YXZ",angles=rotation[i],degrees=True).as_matrix()
            bone_matrix=np.linalg.inv(bone_rest_matrix[i]) @ bone_matrix @ bone_rest_matrix[i]
            tmp=Rotation.from_matrix(bone_matrix).as_euler("ZXY",degrees=False)
