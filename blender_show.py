...
class MocapBlenderOperator(bpy.types.Operator):
        ...
        if event.type == "TIMER":
            ...
            if len(recv_data.strip()) == 0:
                ...
                for i, b in enumerate(bone_names):
                    ...
                    # if b in ['R_Collar', 'R_Shoulder','R_Elbow','R_Wrist','L_Collar', 'L_Shoulder','L_Elbow','L_Wrist']: # SMPL
                    if b in ['right_collar', 'right_shoulder', 'right_elbow', 'right_wrist', 'left_collar', 'left_shoulder', 'left_elbow', 'left_wrist']:  # SMPLX
                        bone0.rotation_mode = "XYZ"  # ZYX is the best
                ...
                self.skeleton1.location = x/100, -z/100, y/100
                print(f"x: {x} ,y: {y} ,z: {z} ")
    ...
    def modal_single(self, context, event):
        if (event.type in {'ESC', 'SPACE'}) or self.stop == True:
            self.cancel(context)
            return {'CANCELLED'}
